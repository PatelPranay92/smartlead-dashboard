        ...
        <div className="space-y-2">
          {payload.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: item.color || '#6366f1' }} />
              <p className="text-sm font-black text-gray-900 dark:text-white tabular-nums">
                {item.value}
                <span className="text-[10px] font-medium text-gray-500 ml-1">{item.name}</span>
              </p>
            </div>
          ))}
        </div>
    ...
    <Card className="group">
      ...
      <ResponsiveContainer width="100%" height={240}>
        ...
        >
          ...
          <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" horizontal={true} vertical={false} opacity={0.3} />
      ...
      <ResponsiveContainer width="100%" height={240}>
        <PieChart>
          ...
          <Legend
            ...
            height={36}
            formatter={(value) => (
              <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 ml-1">{value}</span>
            )}
            iconType="circle"
          />
        </PieChart>
